...
import { getLinkNavigateFunction, type LinkActionOptions, type LinkNavigateFunction, parseBooleanAttribute } from '~/models/link.model.js';
...
export const links: Action<HTMLElement, LinksActionOptions | undefined> = (node: HTMLElement, options: LinksActionOptions | undefined = {}) => {
  ...
  try {
    navigate = getLinkNavigateFunction(options.options);
  } catch (error) {
